...
const debug = require('debug')('job:manager:queue');
...
class QueueManager extends EventEmitter {
  ...
  /**
   * Adds a job to a queue with the given name. Relies on QueueManager.getQueue which means the queue will already be
   * created. If the queue needs to have special options, make sure the queue is created by using getQueue/createQueue
   * first.
  ...
  enqueue(name, data, opts) {
    debug('enqueue', name);
    this.getQueue(name).add(data, opts);
    return true;
  }
   ...
   * @param  {Object} options Options to pass to bull. Options here will override any options provided by default.
   ...
   * connections, but needs to create seperate bclient instances for each queue. This means that the number of clients

...
class QueueManager extends EventEmitter {
  ...
  /**
   * Alias for `getQueue` to create or fetch a queue with custom options.
   * @param {string} name The name of the queue.
   * @param {BullOpts} [options] Optional bull queue options overriding defaults.
   * @return {BullQueue} Bull queue instance.
   */
  createQueue(...args) {
   ...
   * @param  {Object} extraOpts Additional options, e.g. `maxListeners` (defaults to 0).
   ...
   */
  createClient(type, redisOpts, extraOpts = {}) {
    ...
    switch (type) {
      ...
      default:
        ...
        const maxListeners = _.get(extraOpts, 'maxListeners', 0);
        /* eslint-enable no-case-declarations */
        redis.setMaxListeners(maxListeners);
        return redis;
    }
  }
}
